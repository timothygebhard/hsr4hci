...
if __name__ == '__main__':
    ...
    for weight_mode, stacking, add_planet_col in \
            product(['default', 'weighted', 'train_test'],
                    [25, 50, 100],
                    [True, False]):
        ...
        # Set stacking parameter
        file_path = config['dataset']['file_path']
        file_path = file_path.replace('STACKING', str(stacking))
        config['dataset']['file_path'] = file_path
        ...
        experiment_name = (f'weight_mode-{weight_mode}__'
                           f'stacking-{int(stacking)}__'
                           f'add_planet_column-{str(add_planet_col).lower()}')
